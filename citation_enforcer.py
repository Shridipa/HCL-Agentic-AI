...
def verify_and_enforce_citations(answer, metadata_list):
    ...
    # Remove invalid citations instead of rejecting the whole answer
    valid_pages = [str(m.get("page_number")) for m in metadata_list]
    
    # Simple regex to find and remove page citations that are not in valid_pages
    ...
    # Re-check if it still has a citation after cleaning
